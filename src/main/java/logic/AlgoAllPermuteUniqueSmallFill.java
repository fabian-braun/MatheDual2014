...
public class AlgoAllPermuteUniqueSmallFill extends AbstractAlgo implements
		Runnable {
	private static Logger log = LoggerFactory
			.getLogger(AlgoAllPermuteUniqueSmallFill.class);
	...
	public AlgoAllPermuteUniqueSmallFill(Task t) {
		this.task = t;
		maxFlank = 0;
		int surfaceArea = 0;
		for (byte i = 1; i < 10; i++) {
	...
	public void run() {
		...
		for (int x = minFlank, diff = 0; x < maxFlank; x++, diff++) {
			log.debug("span Thread for length(x)=" + x + " and length(y)="
					+ (maxFlank - diff));
			AlgoPermuteUniqueDeepestSmallFill ap = new AlgoPermuteUniqueDeepestSmallFill(
					task, x, maxFlank, (byte) 3);
